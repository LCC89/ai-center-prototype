...
(function() {
    ...
    $(window.document).ready(function() {
        ...
        // Use event delegation for better compatibility with hosted environments
        $(document).off('mousedown.sitemapNode').on('mousedown.sitemapNode', '.sitemapPageLink', function(e) {
            console.log('Sitemap node clicked:', $(this).attr('nodeUrl'));
            node_click.call($(this).parent()[0], e);
        });
        
        // Also try to intercept click events directly on sitemap nodes for Axhub compatibility
        setTimeout(function() {
            console.log('Re-binding sitemap click events...');
            $('.sitemapPageLink').each(function() {
                var $link = $(this);
                var nodeUrl = $link.attr('nodeUrl');
                if (nodeUrl && (nodeUrl.indexOf('ai中枢-') === 0 || nodeUrl.indexOf('api key') === 0)) {
                    console.log('Found custom page link:', nodeUrl);
                    // Remove existing click handlers and add our own
                    $link.off('click.axhub').on('click.axhub', function(e) {
                        console.log('Custom page link clicked:', nodeUrl);
                        e.preventDefault();
                        e.stopPropagation();
                        node_click.call($link.parent()[0], e);
                        return false;
                    });
                }
            });
        ...
        // Monitor URL changes for Axhub compatibility (Axhub uses query params instead of hash)
        ...
        setInterval(function() {
            ...
            if (currentUrl !== lastUrl) {
                ...
                // Check if the URL contains a custom page reference
                var urlParams = new URLSearchParams(window.location.search);
                var pageParam = urlParams.get('p');
                console.log('Page param from URL:', pageParam);
                
                if (pageParam) {
                    // Decode the page parameter
                    var decodedPage = decodeURIComponent(pageParam);
                    console.log('Decoded page:', decodedPage);
                    
                    // Check if it's a custom page
                    if (decodedPage.indexOf('ai中枢-') === 0 || decodedPage.indexOf('api key') === 0) {
                        console.log('Detected custom page in URL:', decodedPage);
                        var nodeUrl = decodedPage + '.html';
                        ...
                        if (mainFrame) {
                            ...
                            var queryIndex = currentHref.indexOf('?');
                            var baseHref = queryIndex > -1 ? currentHref.substring(0, queryIndex) : currentHref;
                            var lastSlashIndex = baseHref.lastIndexOf('/');
                            if (lastSlashIndex > baseHref.indexOf('://') + 2) {
                            ...
                            var fullUrl = baseHref + encodeURI(nodeUrl);
                            console.log('Loading custom page from URL monitor:', fullUrl);
                            
                            $(mainFrame).off('load.customPage').on('load.customPage', function() {
                                showMainPanel();
                            });
        ...
        // Check initial URL on page load (for direct access to custom pages)
        setTimeout(function() {
            var urlParams = new URLSearchParams(window.location.search);
            var pageParam = urlParams.get('p');
            console.log('Initial page check, pageParam:', pageParam);
            
            if (pageParam) {
                var decodedPage = decodeURIComponent(pageParam);
                console.log('Initial decoded page:', decodedPage);
                
                if (decodedPage.indexOf('ai中枢-') === 0 || decodedPage.indexOf('api key') === 0) {
                    console.log('Initial load of custom page:', decodedPage);
                    var nodeUrl = decodedPage + '.html';
                    ...
                    if (mainFrame) {
                        ...
                        var fullUrl = baseHref + encodeURI(nodeUrl);
                        console.log('Loading initial custom page:', fullUrl);
    ...
    function node_click(event) {
        // Get nodeUrl from the clicked element or its child
        var nodeUrl;
        if ($(this).hasClass('sitemapPageLink')) {
            nodeUrl = $(this).attr('nodeUrl');
        } else {
            nodeUrl = $(this).children('.sitemapPageLink').attr('nodeUrl');
        }
        console.log('node_click called with nodeUrl:', nodeUrl);
        // Check if it's a custom HTML page (non-Axure page)
        var isCustomPage = nodeUrl === 'model_mgmt.html' || nodeUrl === 'api_key_mgmt.html' || nodeUrl === 'api_reference.html';
        
        if (isCustomPage) {
            // For custom pages, directly set iframe src without Axure navigation
            hideMainPanel();
            $('#sitemapTreeContainer').find('.sitemapHighlight').removeClass('sitemapHighlight');
            ...
            var mainFrame = document.getElementById('mainFrame');
            if (mainFrame) {
                // Add load event listener to show panel after custom page loads
                $(mainFrame).off('load.customPage').on('load.customPage', function() {
                    showMainPanel();
                });
                
                // For hosted environments like Axhub, we need to use the correct base URL
                var fullUrl = nodeUrl;
                var currentHref = window.location.href;
                
                // Extract base URL - handle both hash and query parameter based URLs
                // Axhub uses: https://axhub.im/ax11/5b2ce9ca3446916e/?g=1&id=xxx&p=xxx
                // Local uses: file:///.../index.html or http://localhost/...
                var baseHref = currentHref;
                
                // Remove hash fragment
                var hashIndex = baseHref.indexOf('#');
                if (hashIndex > -1) {
                    baseHref = baseHref.substring(0, hashIndex);
                }
                
                // Remove query parameters
                var queryIndex = baseHref.indexOf('?');
                if (queryIndex > -1) {
                    baseHref = baseHref.substring(0, queryIndex);
                }
                
                // Remove any file name after the last slash to get the directory
                var lastSlashIndex = baseHref.lastIndexOf('/');
                if (lastSlashIndex > baseHref.indexOf('://') + 2) {
                    baseHref = baseHref.substring(0, lastSlashIndex + 1);
                }
                
                // If baseHref doesn't end with /, add it
                if (!baseHref.endsWith('/')) {
                    baseHref += '/';
                }
                
                // For Axhub and other hosted environments, we need to encode the filename
                // to handle Chinese characters properly
                var encodedNodeUrl = encodeURI(nodeUrl);
                fullUrl = baseHref + encodedNodeUrl;
                
                // Debug logging
                console.log('Loading custom page:');
                console.log('  nodeUrl:', nodeUrl);
                console.log('  encodedNodeUrl:', encodedNodeUrl);
            ...
            // Update URL hash for local environment, or just update internal state for Axhub
            try {
                $axure.player.setVarInCurrentUrlHash('p', nodeUrl.substring(0, nodeUrl.lastIndexOf('.html')));
            } catch(e) {
                // Ignore errors in hosted environments
            }
            return;
        }
        hideMainPanel();
        $('#sitemapTreeContainer').find('.sitemapHighlight').removeClass('sitemapHighlight');
        $(this).parent().addClass('sitemapHighlight');
        $axure.page.navigate(nodeUrl, true);
    }
